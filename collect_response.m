function [resp, t, code]  = collect_response(settings,delay)
% resp returns 1 if keyIsDown, 0 if it is not, 2 if experimenter pressed ESCAPE
% settings.port : 'keyboard', or precised (parallel, etc)
 
ShowCursor()

if ~isfield(settings,'port'),            settings.port = 'keyboard';end
...
while (sum(resp) == 0 || isempty(code)) && t <= delay
    t           = GetSecs;
    %experimenter
    [exp_resp, exp_t, exp_code]       = KbCheck;          % get settings.timing and resp1 from keyboard
    % translate
    ...
    if ~isempty(exp_code)
        switch exp_code
            ...
            case settings.escape, sca; resp = 2; break;
        end
    end
    
    [resp, ~, code]       = KbCheck;          % get settings.timing and resp1 from keyboard
    % translate
    code = KbName(code);
    % only take first response if multiple responses
...
if settings.until_release
    ...
    while sum(resp_release) ~= 0
        [resp_release, ~, name] = KbCheck;          % get settings.timing and resp1 from keyboard
